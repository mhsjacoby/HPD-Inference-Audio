'''
load_saved_models_maggieEdits.py
Author: Sin Yong Tan - October 5, 2020
Edited by Maggie Jacoby
This version reads the saved scaler .pkl file
Uses functions from load_data.py
...
model = model_from_json(open(os.path.join(path, 'model-94_96/CNN_model.json')).read())
model.load_weights(os.path.join(path, 'model-94_96/CNN_weights.h5'))
# model.summary()
...
# load the scaler
scaler = pickle.load(open(os.path.join(path, 'model-94_96/scaler.pkl'), 'rb'))
# Scaling input
